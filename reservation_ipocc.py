...
info_ipo_ex,info_date_ex, key_pair = info_ipo_ex() # login info, reserve date, running key for macro from ipo_reserve_order.xlsx
...
if pass_cal() == key_pair['Pass_key']:
    ...
    " test part"
    job3 = schedule.every().day.at("08:00").do(good_luc_from_ipo)
    job4 = schedule.every().day.at("09:00").do(lambda: reserve_ipo3(info_ipo_ex,info_date_test_afer14days(), reserve_try_cnt  = 9,reserve_able_cnt = 1, reserve_type='test', multi_date = False)  )
    job5 = schedule.every().day.at("12:00").do(
        lambda: reserve_ipo3(info_ipo_ex, info_date_test2(), reserve_try_cnt=9, reserve_able_cnt=1,
                             reserve_type='test', multi_date=False))

    "real part"
    # job3 = schedule.every().day.at("08:30").do(good_luc_from_ipo)
    # job_real1 = schedule.every().day.at("10:00").do(lambda:  reserve_ipo3(info_ipo_ex,info_date_ex, reserve_try_cnt  = 9,reserve_able_cnt = 2, reserve_type='real', multi_date = False) )
