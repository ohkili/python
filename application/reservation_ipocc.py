...
# reserve_able_cnt = 3
# reserve_type='test'
def reserve_ipo4_mobile(loginfo,info_date2, reserve_try_cnt  = 9,reserve_able_cnt = 3, reserve_type='test', multi_date = False):
                        ...
                        if status == '예약':
                            ...
                            try:
                                ...
                                if reserve_type == 'real':
                                    ...
                                    #         (By.XPATH, "//div[@class = 'reservation_table time_table']"))
                                    # )
                                    t = 0
                                    while(t<3):
                                        driver.implicitly_wait(1)
...
if pass_cal() == key_pair['Pass_key']:
    if run_mode['mode'] == 'test':
        ...
        try:
            reserve_ipo4_mobile(info_ipo_ex1, info_date_ex1, reserve_try_cnt=9, reserve_able_cnt=2, reserve_type= run_mode['mode'] ,
                                multi_date=False)
            reserve_ipo4_mobile(info_ipo_ex2, info_date_ex2, reserve_try_cnt=9, reserve_able_cnt=2, reserve_type=run_mode['mode'] , multi_date=False)
        except:
            pass
        " test part"
        job2 = schedule.every().day.at("08:00").do(good_luc_from_ipo)
        job3 = schedule.every().day.at("09:00").do(lambda: reserve_ipo4_mobile(info_ipo_ex1,info_date_test_afer14days(), reserve_try_cnt  = 9,reserve_able_cnt = 2, reserve_type=run_mode['mode'] , multi_date = False)  )
        job4 = schedule.every().day.at("09:00").do(lambda: reserve_ipo4_mobile(info_ipo_ex2,info_date_ex2, reserve_try_cnt  = 9,reserve_able_cnt = 2, reserve_type=run_mode['mode'] , multi_date = False)  )
        job5 = schedule.every().day.at("12:00").do(
            lambda: reserve_ipo4_mobile(info_ipo_ex1, info_date_test2(), reserve_try_cnt=9, reserve_able_cnt=2,reserve_type=run_mode['mode'], multi_date=False))
